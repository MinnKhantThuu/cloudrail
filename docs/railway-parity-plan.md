...
Updated: 2026-09-09  
Status: **UX-0 through UX-4 and UX-5.1–UX-5.2 are implemented and verified; UX-5.3 TCP proxy is next.**
...
- [x] **UX-5.1:** Plain/secret/reference variable types၊ same-environment target selector၊ dependency edge၊ deploy-time recursive resolution၊ rename/delete/cycle guards။ Local PostgreSQL/browser and fresh Ubuntu real-container proof passed; stale legacy browser assertions were corrected and the full suite passed in CI run 34306318618။
- [x] **UX-5.2:** Stable environment-scoped private DNS aliases၊ internal/public URL choice၊ generated/custom domain and target-port flow။ Environment isolation, HTTP route remove/restore, custom domain, regression browser/runtime, maintenance and recovery proof passed in CI run 34306318618။
