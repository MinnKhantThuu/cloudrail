...
Final alpha.3 candidate also rejects composed symlink escapes, symlink cycles and archive-member parent traversal; portable internal symlink chains remain supported. Targeted archive-boundary tests passed locally. The transfer secret is scoped only to decrypting the fixture, outside the restore/application step. Final candidate CI/package verification follows.
