...
## UX-4.5b MongoDB template — pending hosted proof

The versioned template catalog and canvas now include MongoDB 8.0.29 with a digest-pinned image, generated encrypted root credentials, a private `MONGO_URL` binding, authenticated Docker health check and managed `/data/db` volume. Logical archive backup and same-version empty-target restore use `mongodump` and `mongorestore` from the pinned image. API integration, full Go/vet, frontend build and mocked Chrome creation/settings checks must pass locally. A disposable real document must survive restart, backup/restore and a rejected nonempty-target restore before completion.
