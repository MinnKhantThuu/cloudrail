...
Updated: 2026-09-11. ဒီဖိုင်ကို project scope၊ phase အစဉ်နဲ့ progress အတွက် အဓိကစာတမ်းအဖြစ် သုံးမယ်။ အရင် `M1/M2/...` notes တွေနဲ့ ကွဲရင် ဒီ roadmap အတိုင်းလိုက်မယ်။
...
| Phase 6 — Linode pilot | Owner + Railpack app၊ failure/reboot/separate-host restore အောင်; ပထမ 48-hour window မှာ network outage ဖြစ်ပြီး billing evidence မရသဖြင့် recovery + clean observation pending |
| Phase 7 — release preparation | Source + Linux amd64/arm64 archives, CI definitions, docs ပြီး; final local checks အောင် |
| UX | Railway plan + generic canvas graph API + actual pan/zoom/drag canvas shell locally ပြီး; full resource creation/operation flow မပြီးသေး |
| Git / hosting | **[MinnKhantThuu/cloudrail](https://github.com/MinnKhantThuu/cloudrail) public source တင်ပြီး**; alpha.4 release တင်ပြီး; Linode control plane previously verified but unreachable at final observation |
...
`New project → environment ရွေး → canvas ပေါ် New resource → source/workload ရွေး → variables/runtime settings → Build/Deploy → logs/URL ကြည့်`
...
| **2 — Workspace & core UX** | Railway ရဲ့ resource model နဲ့နေ့စဉ် flow အတိုင်း ပြန်တည်ဆောက်ခြင်း | Project canvas၊ generic compute/data/storage resources၊ links၊ resource drawer | **UX-0–UX-3 နဲ့ UX-4.1–UX-4.4 ပြီး; UX-4.5 database templates next** |
...
| **6 — Linode pilot** | ကိုယ်ပိုင် application တစ်ခုနဲ့တကယ်သုံးစမ်းခြင်း | Linode ပေါ် end-to-end deployed app နဲ့ measured cost | **Core operations အောင်; 48-hour window elapsed but availability/billing gate failed; recovery + clean rerun pending** |
...
**2026-09-09 user review အရ Phase 2 ကိုပြန်ဖွင့်ထားသည်။ ပြဿနာက list/grid တစ်ခုတည်းမဟုတ်ဘဲ `Application | PostgreSQL` နှစ်မျိုးပဲရှိသည့် product model ဖြစ်သည်။ Railway ရဲ့ service/source/workload/template/volume/bucket/reference/environment flow ကို [Railway-inspired product plan](railway-parity-plan.md) တွင် UX-0 မှ UX-9 အထိရေးထားသည်။ UX-1 resource graph/API နှင့် UX-2 actual canvas shell ကို local code/browser အထိပြီးထားသည်။ Phase 6 infrastructure evidence ကိုမဖျက်ဘဲ UX-3 compute creation/runtime ကိုဆက်မည်။**
...
**2.4 Railway-inspired product and canvas recovery — UX-0 through UX-4 and UX-5.1 complete; UX-5.2 selected**
...
- [x] **UX-3:** GitHub/Docker/Empty source + web/worker/cron creation and runtime။
...
- [ ] **UX-9:** Public Linode pilot update, docs/release and separate user UX acceptance।
  - [x] UX-9.1 verified main commit `c15158a` ကို guarded update နဲ့ public Linode သို့တင်ပြီး existing owner/session/database နှင့် active sample deployment ကိုထိန်းထားနိုင်သည်။
  - [x] UX-9.2 public dashboard/API/browser canvas နဲ့ resource palette ကိုစစ်ပြီး deployment checkpoint docs ကိုပြင်ထားသည်။
  - [ ] UX-9.3 versioned release နဲ့ user UX acceptance။

**Next selected substep:** UX-5.3 — bounded TCP proxy allocation, lifecycle/collision guards and real external connection proof။

**UX-2 visible outcome:** Project overview သည် card grid/list မဟုတ်တော့ဘဲ actual service/database/volume/bucket resources နဲ့ real relations ကို canvas ပေါ်တွင်မြင်၊ နေရာရွှေ့၊ click လုပ်ပြီး drawer ထဲဝင်နိုင်သည်။ ဒီ canvas ကို public Linode သို့တင်ပြီး browser မှာစစ်ထားသည်; user review မရသေး။
...
**6.1 Deployment preparation:** Target Linode ရဲ့ Ubuntu version၊ CPU/RAM/disk၊ SSH access၊ Cloud Firewall၊ public IPv4၊ registry၊ backups နဲ့ current plan cost ကိုစစ်မယ်။ Target `172.104.38.63` သည် Ubuntu 24.04.4 x86-64, 2 CPU, 3.8 GiB RAM, 79 GiB root disk ဖြစ်သည်။ Dedicated root SSH key ကို verify လုပ်ပြီး Cloud Firewall မှာ 22/80/443 allow, default inbound drop ဖြစ်သည်။
...
- **Current:** Phase 2.4 UX-0 through UX-4 and UX-5.1–UX-5.2 are implemented and verified. UX-5.3 TCP proxy is selected next. UX-9.1–UX-9.2 updated the public Linode to verified commit `c15158a`; the canvas and resource palette passed public verification before the pilot became unreachable. The first 48-hour window elapsed, but 36-hour, 42-hour and final samples timed out on HTTPS and SSH, and authenticated billing evidence was unavailable. Linode recovery/state verification, a clean observation rerun, user UX acceptance, a new versioned release, GitHub App delivery and certificate renewal remain pending.
...
- **UX:** UX-2 renders the environment graph as a pan/zoom/fit canvas with persisted node drag, URL-restored selection, actual reference/attachment edges, click drawers and shared create entry points. UX-3 adds GitHub/Docker/empty sources, web/worker/cron modes, schedules, commands and restart policies. UX-4 provides PostgreSQL/Redis/MySQL/MongoDB templates, standalone volumes/backups and S3-compatible buckets. UX-5.1 adds typed variables, dependency edges and deploy-time reference resolution. UX-5.2 adds stable environment-scoped private hostnames, public route on/off, generated/custom domains and next-deploy target ports. CI run 34306318618 passed all checks, real-container reference/network/data/storage suites, live dashboard browser, maintenance and independent-host recovery. The public canvas update passed guarded maintenance and live browser/API checks. TCP exposure and user review remain pending.
...
- **Account gap:** Linode access, temporary pilot DNS/ACME and owner setup had completed, but current host/network recovery and authenticated billing access are now required. GitHub App installation/delivered webhook and a repository dedicated to its push test are pending. A user-owned branded domain remains optional before wider use.
...
- **Phase 6 preparation:** Linode host access, inventory, Docker prerequisites, Cloud Firewall and temporary pilot DNS were complete. The measured window ran 48 hours 8 minutes; start and 30-hour samples were healthy, while 36-hour, 42-hour and final samples could not reach 22/443. Actual billing evidence was unavailable because the account view required login. Recover/reverify the host, then rerun the observation. AWS Lightsail remains an optional provider reference.
...
- **Next external inputs:** Check the Linode power/firewall/provider status and restore SSH/HTTPS reachability, then expose authenticated billing evidence for the replacement observation. Complete GitHub sudo/passkey authentication separately, then create/install the prepared least-privilege GitHub App on the `cloudrail` repository and prove one push delivery. Secrets stay outside chat. See [Linode pilot](linode-pilot.md), [release gates](release.md), [current verification](verification-current.md).
...
| Linode pilot, real app flow, off-server recovery and 48-hour measured operation/cost | Public app, failed-build preservation, reboot, 503 MiB off-server backup and temporary empty-host restore/new deploy passed; first 48-hour window ended with repeated HTTPS/SSH timeouts and no authenticated charge evidence | Recovery and clean rerun required |
| Final owner UX walkthrough | Automated/visual checks exist, but no owner acceptance was supplied | Pending owner review |

Recover the Linode network/host and verify retained owner, database and deployment state. Then capture authenticated billing evidence and restart a clean 48-hour observation. GitHub sudo/passkey authentication is still required for the separate least-privilege App push-delivery gate. Continue with the [Linode pilot runbook](linode-pilot.md). This state does not claim certificate renewal, production/beta readiness or an external security audit.
