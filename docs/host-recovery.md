...
The CI rehearsal uses a new Docker daemon with a separate data root after fencing/removing the disposable source containers. It checks owner/session, private identity, database rows, volumes, registry artifacts, encrypted bindings, backup downloads, certificate-volume bytes, stopped state and a new deployment. It runs on one Ubuntu runner/kernel; it does not substitute for an independent physical VPS, public TLS or the AWS pilot. No private backup/fixture is uploaded as a CI artifact. See [verification](verification-current.md) for recorded results.
