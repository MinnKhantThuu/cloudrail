...
**Status: early alpha (`0.1.0-alpha.2`).** Designed for **one owner, one Linux node and trusted repositories**. Local deployment, recovery, database and browser journeys have been tested, along with a fresh Ubuntu CI Docker quickstart. Public Ubuntu installation, DNS-issued HTTPS, a real GitHub App push and an AWS pilot still require end-to-end verification. Public source availability does not mean production readiness. See [verification](docs/verification-current.md).
