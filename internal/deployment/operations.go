...
func (c Settings) Validate() error {
	...
	if c.Kind == "postgres" {
		min = 256
	} else if c.Kind == "mysql" || c.Kind == "mongo" {
		min = 512
	}
	if c.MemoryMB < min || c.MemoryMB > 4096 || c.CPUMillis < 100 || c.CPUMillis > 4000 {
		return errors.New("use 64–4096 MB (256+ for PostgreSQL, 512+ for MySQL/MongoDB) and 0.1–4 CPUs")
	}
	if c.MountPath != "" {
...
func (s *Store) BindDatabase(ctx context.Context, database, target, name string) error {
	...
	case "mongo":
		connection = vars["MONGO_URL"]
		targetVariable = "MONGO_URL"
	}
	if connection == "" {
