...
function App({ onLogout }: { onLogout: () => void }) {
 ...
 const [serviceKind,setServiceKind]=useState('http');
  ...
  const beginCreate = (intent: CreateIntent) => {
    setCreateIntent(intent); setCreateOpen(false); setServiceKind(intent === 'postgres' ? 'postgres' : 'http'); setName(''); setFormError(''); setModal('service');
  };
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (modal === 'service' && project) { const s = await request<Service>(`/api/projects/${project.id}/${serviceKind==='postgres'?'databases':'services'}`, { name,environment }); setServiceID(s.id); setDeploymentID(''); setLocationResource(`service:${s.id}`); setTab(s.settings.kind==='postgres'?'settings':createIntent==='github'?'source':'deployments'); openImageDeploy=createIntent==='image'; }
