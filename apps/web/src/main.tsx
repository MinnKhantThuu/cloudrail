...
const dataName = (service?: Service) => service?.template === 'redis' ? 'Redis' : service?.template === 'mysql' ? 'MySQL' : 'PostgreSQL';
const dataPort = (service?: Service) => service?.template === 'redis' ? 6379 : service?.template === 'mysql' ? 3306 : 5432;
...
function App({ onLogout }: { onLogout: () => void }) {
 ...
 const [bucketPathStyle,setBucketPathStyle]=useState(true);
  ...
  const beginCreate = (intent: CreateIntent) => {
    setCreateIntent(intent); setCreateOpen(false); setServiceKind(intent === 'postgres' || intent === 'redis' || intent === 'mysql' || intent === 'volume' || intent === 'bucket' ? intent : 'http');
