...
import { Activity, ArrowDownToLine, ArrowRight, Box, Check, ChevronDown, ChevronRight, Circle, Clock3, Database, ExternalLink, Folder, GitBranch, Layers3, LoaderCircle, LogOut, Plus, Radio, Rocket, Server, ShieldCheck, Terminal, X } from 'lucide-react';
import './styles.css';
...
type Service = { url:string;settings:{kind:string;memoryMB:number;cpuMillis:number;mountPath:string;volumeName:string;network:string};id: string; projectId: string; name: string; environment: string; host: string; activeId: string; desiredState: string; resourceKind:string; workloadMode:string; template:string; createdAt: string };
...
function App({ onLogout }: { onLogout: () => void }) {
 ...
 const [workloadMode,setWorkloadMode]=useState<'web'|'worker'|'cron'>('web');
  ...
  const open = (kind: Modal, previous?: Deployment) => {
    requestKey.current=crypto.randomUUID();setName(''); setFormError(''); setImage(previous?.image || (service?.settings.kind==='postgres'?(active?.image||deployments[0]?.image||''):'')); setPort(String(previous?.port || (service?.settings.kind==='postgres'?5432:80))); setHealth(previous?.healthPath || '/'); setModal(kind);
  };
  ...
  const beginCreate = (intent: CreateIntent) => {
    setCreateIntent(intent); setCreateOpen(false); setServiceKind(intent === 'postgres' ? 'postgres' : 'http');
    setSourceType(intent === 'github' ? 'github' : intent === 'image' ? 'image' : 'empty');
    setWorkloadMode(intent === 'worker' ? 'worker' : intent === 'cron' ? 'cron' : 'web');
    setName(''); setFormError(''); setModal('service');
  };
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (modal === 'service' && project) { const s = await request<Service>(serviceKind==='postgres'?`/api/projects/${project.id}/databases`:`/api/projects/${project.id}/resources`, serviceKind==='postgres'?{name,environment}:{name,environment,sourceType,workloadMode}); setServiceID(s.id); setDeploymentID(''); setLocationResource(`service:${s.id}`); setTab(s.settings.kind==='postgres'?'settings':sourceType==='github'?'source':'deployments'); openImageDeploy=sourceType==='image'; }
