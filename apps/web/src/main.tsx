...
import { Activity, ArrowDownToLine, ArrowRight, Box, Check, ChevronDown, ChevronRight, Circle, Clock3, Database, ExternalLink, Folder, GitBranch, HardDrive, Layers3, LoaderCircle, LogOut, Plus, Radio, Rocket, Server, ShieldCheck, Terminal, X } from 'lucide-react';
import './styles.css';
...
const dataName = (service?: Service) => service?.template === 'redis' ? 'Redis' : 'PostgreSQL';
const dataPort = (service?: Service) => service?.template === 'redis' ? 6379 : 5432;
...
function App({ onLogout }: { onLogout: () => void }) {
 ...
 const [workloadMode,setWorkloadMode]=useState<'web'|'worker'|'cron'>('web');
  ...
  const beginCreate = (intent: CreateIntent) => {
    setCreateIntent(intent); setCreateOpen(false); setServiceKind(intent === 'postgres' || intent === 'redis' || intent === 'volume' ? intent : 'http');
