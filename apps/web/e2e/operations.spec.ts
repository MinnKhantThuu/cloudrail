...
test('create a PostgreSQL service and manage a real backup in the dashboard',async({page})=>{
 ...
 await page.getByRole('button',{name:result.project.name,exact:true}).click();await page.locator('.project-header').getByRole('button',{name:'New resource',exact:true}).click();await page.getByRole('dialog',{name:'Add to your canvas'}).getByRole('button',{name:/PostgreSQL/}).click();const name='browser-db-'+Date.now().toString().slice(-5);const dialog=page.getByRole('dialog');await dialog.getByLabel('Resource name').fill(name);await dialog.getByRole('button',{name:'Create resource',exact:true}).click();
